...
local Main_1_left = Main_1_Page:section({name = "Teleport",side = "left",size = 500})
local Main_1_right = Main_1_Page:section({name = "Misc",side = "right",size = 250})

Main_1_right:button({name = "infinfiniteyield",def = false,callback = function()
    loadstring(game:HttpGet('https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source'))()
end})

Main_1_right:button({name = "Walkspeed",def = false,callback = function()
...
Main_1_right:button({name = "KILL ALL (equip combat)",def = false,callback = function()
    local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer

for _, player in ipairs(Players:GetPlayers()) do
    if player ~= LocalPlayer and player.Character then  -- Ensure it's not the local player and the character exists
        for i = 1, 5 do  -- Repeat 5 times
            ...
            -- Invoke the punch remote for each player
            LocalPlayer.Character.Combat.LocalScript.punched:InvokeServer(unpack(args))
        end
    end
end
end})

Main_1_right:button({name = "KILL ALL LOOP (equip combat)",def = false,callback = function()
while wait(1) do
    local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
...
Main_1_left:button({name = "Floor 1",def = false,callback = function()
    local args = {
        [1] = Vector3.new(-786.7779541015625, 66.16873168945312, 751.1465454101562)
    }
    
    game:GetService("ReplicatedStorage"):WaitForChild("SavePositionEvent"):FireServer(unpack(args))
    wait(0.5)
    game.Players.LocalPlayer.Character.Humanoid.Health = 0
end})

Main_1_left:button({name = "Floor 2",def = false,callback = function()
    local args = {
        [1] = Vector3.new(-1163.0958251953125, 1125.173583984375, 1385.4771728515625)
    }
...
Main_1_left:button({name = "Floor 3",def = false,callback = function()
    local args = {
        [1] = Vector3.new(-1590.9091796875, 2219.33642578125, 2103.712646484375)
    }
...
Main_1_left:button({name = "Floor 4",def = false,callback = function()
    local args = {
        [1] = Vector3.new(-1948.484130859375, 3299.739990234375, 2999.834228515625)
    }
...
Main_1_left:button({name = "Floor 5",def = false,callback = function()
    local args = {
        [1] = Vector3.new(-2957.02001953125, 328.4762878417969, 4366.26171875)
    }
...
Main_1_left:button({name = "Floor 6",def = false,callback = function()
    local args = {
        [1] = Vector3.new(-2114.312255859375, 4120.19921875, 3559.546630859375)
    }
...
Main_1_left:button({name = "Floor 7",def = false,callback = function()
    local player = game.Players.LocalPlayer

    player.Character.HumanoidRootPart.CFrame = CFrame.new(-2502, 5181, 4303)
    
end})

Main_1_left:button({name = "Floor 8",def = false,callback = function()
    local player = game.Players.LocalPlayer

    player.Character.HumanoidRootPart.CFrame = CFrame.new(-2909, 6274, 5046)
    
end})

Main_1_left:button({name = "Floor 9",def = false,callback = function()
    local player = game.Players.LocalPlayer

    player.Character.HumanoidRootPart.CFrame = CFrame.new(-3295, 7346, 5788)
    
end})

Main_1_left:button({name = "Floor 10",def = false,callback = function()
    local player = game.Players.LocalPlayer

    player.Character.HumanoidRootPart.CFrame = CFrame.new(-3489, 7516, 6075)
    
end})

Main_1_right:button({name = "Run",def = false,callback = function()
    local args = {
        [1] = true
    }
    
    game:GetService("Players").LocalPlayer.Character.LocalScript.RemoteEvent:FireServer(unpack(args))
end})

Main_1_right:button({name = "Fly",def = false,callback = function()
    getgenv().Speed = 1
getgenv().Device = "PC"
...
function sFLY()
	repeat wait() until game:GetService("Players").LocalPlayer and game:GetService("Players").LocalPlayer.Character and RootPart and game:GetService("Players").LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
	...
	flyKeyDown = Mouse.KeyDown:Connect(function(KEY)
		if KEY:lower() == 'w' then
			CONTROL.F = (getgenv().Speed)
		elseif KEY:lower() == 's' then
			CONTROL.B = - (getgenv().Speed)
		elseif KEY:lower() == 'a' then
			CONTROL.L = - (getgenv().Speed)
		elseif KEY:lower() == 'd' then 
			CONTROL.R = (getgenv().Speed)
		elseif QEfly and KEY:lower() == 'e' then
			CONTROL.Q = (getgenv().Speed)*2
		elseif QEfly and KEY:lower() == 'q' then
			CONTROL.E = -(getgenv().Speed)*2
		end
	...
	mfly2 = RunService.RenderStepped:Connect(function()
		...
		if game:GetService("Players").LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid") and root and root:FindFirstChild(velocityHandlerName) and root:FindFirstChild(gyroHandlerName) then
			...
			if direction.X > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((getgenv().Speed) * 50))
			end
			if direction.X < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity + camera.CFrame.RightVector * (direction.X * ((getgenv().Speed) * 50))
			end
			if direction.Z > 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((getgenv().Speed) * 50))
			end
			if direction.Z < 0 then
				VelocityHandler.Velocity = VelocityHandler.Velocity - camera.CFrame.LookVector * (direction.Z * ((getgenv().Speed) * 50))
			end
